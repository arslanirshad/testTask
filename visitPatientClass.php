...
class visitPatientClass {
	...
	public function before_save($bean, $event, $arguments) {
		...
		$this->visit_patient_before_method($bean, $event, $arguments);
		// Now we can add as many functions we want to run before_save with using only one logic hook definition for it.
		// Same for after_save and after_relationship_add
	}

	public function after_save($bean, $event, $arguments) {
		$this->afterSave($bean, $event, $arguments);
	}

	public function after_relationship_add($bean, $event, $arguments) {
		$this->afterRelationshipAdd($bean, $event, $arguments);
	}
	...
	public function visit_patient_before_method($bean, $event, $arguments) {
		...
		if (!$arguments['isUpdate']) {
			...
			$main_patient_id = $this->obtainMainPatientId($bean);

			if (!is_null($main_patient_id)) {
				// Check pending Notes and Clinics
				$this->checkPendingNotesAndClinics($main_patient_id);

				// Verify patient's balance
				$patient = $this->verifyPatientAndGet($main_patient_id);

				// Update Visit
				$this->verifyPatientAndUpdateVisit($bean, $patient);

				// Process treatment plan
				$this->processTreatmentPlan($bean, $patient);
			}
		}

		$this->checkVisitStatus($bean);

		if ($arguments['isUpdate']) {
			$isReExam = $this->handleStatusChecks($bean, $arguments);
			$this->handlePendingNotesStatus($bean);
			$this->handleCompletedVisit($bean);
		}

		$bean->has_do_not_adjust = TJ_VisitsUtils::hasDoNotAdjust((Array) $bean);
	}

	private function checkPendingNotesAndClinics($main_patient_id) {
		...
		if ($visitPendingNotes && $visitPendingNotes->id) {
			$clinic = BeanFactory::retrieveBean("TJ_Clinics", $visitPendingNotes->tj_clinics_tj_visits_1tj_clinics_ida, array('disable_row_level_security' => true));
			throw new SugarApiExceptionInvalidParameter('Patient has notes pending at the ' . (($clinic) ? $clinic->name : "") . ' clinic. WC/DC call clinic.');
		}
	}

	private function obtainMainPatientId($bean) {
		$main_patient_id = null;

		if ($bean->load_relationship('contacts_tj_visits_1')) {
			$relatedPatient = $bean->contacts_tj_visits_1->get();
			if (count($relatedPatient)) {
				$main_patient_id = current($relatedPatient);
			} else {
				throw new SugarApiExceptionInvalidParameter('There are no related patients records.');
			}
		}
	...
	private function verifyPatientAndGet($main_patient_id) {
		$patient = BeanFactory::retrieveBean("Contacts", $main_patient_id, array('disable_row_level_security' => true));

		if (!$patient) {
			throw new SugarApiExceptionInvalidParameter('The patient record does not exist.');
		}

		if (floatval($patient->payment_due) > 0 || floatval($patient->balance) > 0) {
	...
	private function verifyPatientAndUpdateVisit($bean, $patient) {
		...
		if ($lastVisitID) {
			...
			if (!$lastVisitBean) {
				throw new SugarApiExceptionInvalidParameter('The last vsit record does not exist.');
			}
	...
	private function processTreatmentPlan($bean, $patient) {
		if ($patient->load_relationship('contacts_tjc_treatmentplans_1')) {
			$timedate = new TimeDate();
			$treatment_end = null;

			$sugarQuery = new SugarQuery();
			$sugarQuery->select(array('t.id', 't.end_date'));
			$sugarQuery->from(BeanFactory::newBean('Contacts'));
			$sugarQuery->join('contacts_tjc_treatmentplans_1', array('alias' => 't'));
			$sugarQuery->where()->equals('t.active', 1);
			$sugarQuery->orderBy('t.end_date');
			$sugarQuery->limit(1);
			$result = $sugarQuery->execute();

			if (count($result)) {
				$planID = $result[0]["id"];
				$treatment_end = $result[0]["end_date"];
			}
			...
			throw new SugarApiExceptionInvalidParameter('Could not load treatment plan relationship.');
		}
	}

	private function checkVisitStatus($bean) {
		global $current_user;
	...
	private function handleStatusChecks($bean, $arguments) {
		$isReExam = false;

		if ($bean->status !== 'Forfeited') {
			$isReExam = $this->isReExam($arguments, true, $bean);
		}
		if ($bean->status === "Pending Notes" && $bean->status !== $bean->fetched_row["status"] && !empty($bean->waiting_queue_time)
		) {
			$seconds = (strtotime($bean->date_modified) - strtotime($bean->waiting_queue_time));
			$bean->average_wait_time = $seconds;
		}
	...
	private function handlePendingNotesStatus($bean) {
	...
	private function handleCompletedVisit($bean) {

		if ($bean->status === "Completed" && $bean->status != $bean->fetched_row["status"]) {

			$this->addUserToVisitRelationship($bean);
			$this->updatePurchaseDC($bean);
			$this->handleVisitOthers($bean);
		}
	}

	private function addUserToVisitRelationship($bean) {
		global $current_user;
		if (!$bean->load_relationship('users_tj_visits_2')) {
			throw new SugarApiExceptionInvalidParameter("Could not load visits relationship.");
		}

		$bean->users_tj_visits_2->add($current_user->id);
	}

	private function handleVisitOthers($bean) {
		$timedate = new TimeDate();

		if (!$bean->load_relationship("tj_visits_tj_visitothers_1")) {
			throw new SugarApiExceptionInvalidParameter("Could not load visit others relationship.");
		}

		$relatedID = $bean->tj_visits_tj_visitothers_1->get();
		$beanOther = BeanFactory::retrieveBean('TJ_VisitOthers', current($relatedID), array('disable_row_level_security' => true));

		if (!$beanOther) {
			throw new SugarApiExceptionInvalidParameter("Could not retrieve bean visit others.");
		}

		// Handle visit info
		$this->handleVisitInfo($bean, $beanOther, $timedate);

		// Handle patient info
		$this->handlePatientInfo($bean, $beanOther);

		// Handle purchase info
		$this->handlePurchaseInfo($bean, $beanOther);

		$beanOther->save();
	}

	private function handleVisitInfo($bean, $beanOther, $timedate) {
		//Visit Info
	...
	private function handlePatientInfo($bean, $beanOther) {
		//Patient info
		if ($bean->load_relationship('contacts_tj_visits_1')) {
			$relatedPatient = $bean->contacts_tj_visits_1->get();
			$main_patient_id = current($relatedPatient);
		...
		if (!is_null($main_patient_id)) {
			...
			throw new SugarApiExceptionInvalidParameter("Could not get main patient record.");
		}
	}

	private function handlePurchaseInfo($bean, $beanOther) {
		//Purchase info
		...
		if (!is_null($purchase)) {
			...
			throw new SugarApiExceptionInvalidParameter("Could not retrieve TJ_Purchases bean.");
		}
	}
	...
	public function afterSave($bean, $event, $arguments) {
		...
		if ($arguments['isUpdate']) {
			...
			if (
				...
				$arguments['dataChanges']['saved_bo']['before'] === false && $arguments['dataChanges']['saved_bo']['after'] === true && $_SESSION['platform'] === "backoffice") {
